...
//
// Purpose:
// - Documents what patterns the TimeWarp.Nuru.Analyzers would flag as errors
// - Ensures the parser validation aligns with analyzer diagnostics
// - Provides examples of common mistakes and their corrections
//
// Each test case shows:
// 1. An invalid pattern that should be rejected
// 2. The corrected version that should parse successfully
// 3. An explanation of why the pattern is problematic
//
...
(
  "NURU005",
  "Multiple parameters after catch-all",
  "exec {*args} {env} {user}",
  "exec {env} {user} {*args}",
...
(
  "NURU006",
  "Duplicate with different positions",
  "move {path} to {path}",
...
(
  "NURU008",
  "Multiple optional with catch-all",
  "run {script?} {env?} {*args}",
  "run {script} {env} {*args}",
