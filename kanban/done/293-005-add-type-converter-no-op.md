...
- [x] `AddTypeConverter(IRouteTypeConverter)` → no-op, return `(TSelf)this` (line 98-103)
- [x] Add comment explaining source gen handles type conversion
- [x] Verify `TypeConverterRegistry` is not used elsewhere at runtime (only used by REPL/completion - separate concern in #293-007)
