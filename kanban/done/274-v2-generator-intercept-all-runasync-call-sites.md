...
- [x] Update `AppModel` record: `InterceptSite` -> `InterceptSites` (ImmutableArray)
- [x] Update `AppModelBuilder` to handle collection of sites
- [x] Update `CombineModels` in `NuruGenerator` to collect all sites
- [x] Update `InterceptorEmitter` to emit multiple `[InterceptsLocation]` attributes
- [x] Verify all 4 minimal intercept tests pass

## Results

**Completed 2024-12-26**

All changes implemented as planned. Key insight: after making generator changes, must clear runfile cache with `ganda runfile cache --clear` to pick up new generator output.
