...
public class EnumSourceTests
{
  ...
  public static async Task Should_ignore_context_parameter()
  {
    ...
    // Act
    List<CompletionCandidate> completions1 = source.GetCompletions(context1).ToList();
    List<CompletionCandidate> completions2 = source.GetCompletions(context2).ToList();
