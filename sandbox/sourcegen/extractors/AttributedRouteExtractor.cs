// sandbox/sourcegen/extractors/AttributedRouteExtractor.cs
...
/// Extracts route definitions from attributed request classes.
/// </summary>
public static class AttributedRouteExtractor
{
  /// <summary>
    ...
    foreach (PropertyInfo prop in properties)
    {
      ...
      foreach (SegmentDefinition segment in segments)
      {
        if (segment is ParameterDefinition param &&
            param.Name.ToLowerInvariant() == propNameLower)
        {
        ...
        {
          if (option.LongForm.ToLowerInvariant() == propNameLower ||
              option.ParameterName?.ToLowerInvariant() == propNameLower)
          {
            if (option.IsFlag)
            {
