...
if (cd tests/test-apps/timewarp-nuru-testapp-delegates && dotnet build -c Release --nologo --verbosity quiet > /dev/null 2>&1); then
    echo -e "${GREEN}✓${NC}"
else
    echo -e "${RED}✗ Build failed${NC}"
    echo "Build error output:"
    (cd tests/test-apps/timewarp-nuru-testapp-delegates && dotnet build -c Release --nologo)
    exit 1
fi

echo -n "  Mediator version (JIT)... "
if (cd tests/test-apps/timewarp-nuru-testapp-mediator && dotnet build -c Release --nologo --verbosity quiet > /dev/null 2>&1); then
    echo -e "${GREEN}✓${NC}"
else
    echo -e "${RED}✗ Build failed${NC}"
    echo "Build error output:"
    (cd tests/test-apps/timewarp-nuru-testapp-mediator && dotnet build -c Release --nologo)
    exit 1
fi

echo -n "  Delegate version (AOT)... "
if (cd tests/test-apps/timewarp-nuru-testapp-delegates && dotnet publish -c Release -r linux-x64 -p:PublishAot=true -o ./aot-output --nologo --verbosity quiet > /dev/null 2>&1); then
    echo -e "${GREEN}✓${NC}"
else
    echo -e "${RED}✗ AOT build failed${NC}"
    echo "Build error output:"
    (cd tests/test-apps/timewarp-nuru-testapp-delegates && dotnet publish -c Release -r linux-x64 -p:PublishAot=true -o ./aot-output --nologo)
    exit 1
fi

echo -n "  Mediator version (AOT)... "
if (cd tests/test-apps/timewarp-nuru-testapp-mediator && dotnet publish -c Release -r linux-x64 -p:PublishAot=true -o ./aot-output --nologo --verbosity quiet > /dev/null 2>&1); then
    echo -e "${GREEN}✓${NC}"
else
...
EXECUTABLE="./tests/test-apps/timewarp-nuru-testapp-delegates/bin/Release/$TARGET_FRAMEWORK/TimeWarp.Nuru.TestApp.Delegates.dll"
START_TIME=$(date +%s.%N)
run_all_tests
END_TIME=$(date +%s.%N)
...
echo "================================="
# Use the DLL with dotnet exec for JIT (apphost may not find runtime)
EXECUTABLE="./tests/test-apps/timewarp-nuru-testapp-mediator/bin/Release/$TARGET_FRAMEWORK/TimeWarp.Nuru.TestApp.Mediator.dll"
START_TIME=$(date +%s.%N)
run_all_tests
END_TIME=$(date +%s.%N)
...
echo "================================="
EXECUTABLE="./tests/test-apps/timewarp-nuru-testapp-delegates/aot-output/TimeWarp.Nuru.TestApp.Delegates"
...
echo "================================="
EXECUTABLE="./tests/test-apps/timewarp-nuru-testapp-mediator/aot-output/TimeWarp.Nuru.TestApp.Mediator"
if [ -f "$EXECUTABLE" ]; then
    START_TIME=$(date +%s.%N)
    run_all_tests
    END_TIME=$(date +%s.%N)
...
echo ""
echo "Binary Sizes:"
if [ -f "./tests/test-apps/timewarp-nuru-testapp-delegates/aot-output/TimeWarp.Nuru.TestApp.Delegates" ]; then
    DELEGATE_SIZE=$(ls -lh "./tests/test-apps/timewarp-nuru-testapp-delegates/aot-output/TimeWarp.Nuru.TestApp.Delegates" | awk '{print $5}')
    echo "- Delegate AOT: $DELEGATE_SIZE"
fi
if [ -f "./tests/test-apps/timewarp-nuru-testapp-mediator/aot-output/TimeWarp.Nuru.TestApp.Mediator" ]; then
    MEDIATOR_SIZE=$(ls -lh "./tests/test-apps/timewarp-nuru-testapp-mediator/aot-output/TimeWarp.Nuru.TestApp.Mediator" | awk '{print $5}')
