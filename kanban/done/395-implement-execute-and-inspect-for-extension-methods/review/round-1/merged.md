...
- Source: general
- Disposition notes: `ResolveServiceForCommand` `global::` fallback now skips `IsInternalType`. generator-42 Command handler injects `IEm42HiddenCmd`; generated source has no `new` / `__svc_` for the internal impl.
...
- Source: general
- Disposition notes: `FindLibSibling` ranks `lib/` TFMs by closeness to the compile stub (then compilation TFM) and walks nearest-first until a method has a real body.
