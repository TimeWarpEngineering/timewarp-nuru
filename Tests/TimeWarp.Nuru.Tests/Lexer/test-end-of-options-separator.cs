 ...
 * 
 * Reason for Deletion:
 * This console-script-style test duplicates coverage for end-of-options separator (--), now centralized in lexer-06-end-of-options.cs.
 * Tests for standalone --, command followed by --, -- with catch-all, git log --, --help as option, complex mixes (e.g., exec --env {e} -- {*cmd}) are covered in Should_tokenize_end_of_options_after_commands, Should_tokenize_end_of_options_in_middle_of_pattern, etc.
 * The structured equivalent is in test-end-of-options-separator-kijaribu.cs (commented/moved if unique).
