...
public class ParameterContextTests
{
  ...
  public static async Task Should_tokenize_typed_parameter()
  {
    ...
    tokens[5].Type.ShouldBe(TokenType.EndOfInput);
  ...
  public static async Task Should_tokenize_catchall_parameter()
  {
    ...
    tokens.Count.ShouldBe(5);
    tokens[0].Type.ShouldBe(TokenType.LeftBrace);
    tokens[1].Type.ShouldBe(TokenType.Asterisk);
  ...
  public static async Task Should_tokenize_combined_type_and_optional()
  {
    ...
    tokens[3].Type.ShouldBe(TokenType.Identifier);
    tokens[3].Value.ShouldBe("int");
    tokens[4].Type.ShouldBe(TokenType.Question);  // Optional marker
  ...
  public static async Task Should_tokenize_enum_values_with_pipes()
  {
    ...
    tokens[4].Type.ShouldBe(TokenType.Pipe);  // First pipe
    tokens[5].Type.ShouldBe(TokenType.Identifier);
    tokens[5].Value.ShouldBe("staging");
    tokens[6].Type.ShouldBe(TokenType.Pipe);  // Second pipe
  ...
  public static async Task Should_detect_nested_braces()
  {
    ...
    tokens.Count.ShouldBe(6);
    tokens[0].Type.ShouldBe(TokenType.LeftBrace);   // First {
    tokens[1].Type.ShouldBe(TokenType.LeftBrace);   // Second {
    tokens[2].Type.ShouldBe(TokenType.Identifier);
    tokens[2].Value.ShouldBe("name");
    tokens[3].Type.ShouldBe(TokenType.RightBrace);  // First }
    tokens[4].Type.ShouldBe(TokenType.RightBrace);  // Second }
    tokens[5].Type.ShouldBe(TokenType.EndOfInput);
  ...
  public static async Task Should_detect_unclosed_brace()
  {
    ...
    tokens[1].Type.ShouldBe(TokenType.Identifier);
    tokens[1].Value.ShouldBe("name");
    tokens[2].Type.ShouldBe(TokenType.EndOfInput);  // No closing brace

    // Verify no closing brace token exists
    tokens.Any(t => t.Type == TokenType.RightBrace).ShouldBeFalse("Should not have closing brace");
