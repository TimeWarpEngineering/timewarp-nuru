...
# Modified test script that times ONLY the execution, not the build
...
echo "======================================"
echo "TimeWarp.Nuru Performance Comparison"
echo "======================================"
echo ""

# Build JIT version first (not timed)
echo "Building JIT version..."
dotnet build --nologo --verbosity quiet -c Release > /dev/null 2>&1

# Build AOT version (not timed)  
echo "Building AOT version..."
dotnet publish --nologo --verbosity quiet -c Release -r linux-x64 -p:PublishAot=true -o ./aot-output > /dev/null 2>&1

echo "Build complete. Starting performance tests..."
echo ""

# Test JIT version
echo "Testing JIT version:"
echo "===================="
EXECUTABLE="./bin/Release/net9.0/TimeWarp.Nuru.TestApp"
START_TIME=$(date +%s.%N)
run_all_tests > /dev/null 2>&1
END_TIME=$(date +%s.%N)
JIT_TIME=$(echo "$END_TIME - $START_TIME" | bc)
echo ""
echo -e "${GREEN}JIT execution time: ${JIT_TIME}s${NC}"
...
echo ""
echo "Testing AOT version:"
echo "===================="
EXECUTABLE="./aot-output/TimeWarp.Nuru.TestApp"
START_TIME=$(date +%s.%N)
run_all_tests > /dev/null 2>&1
END_TIME=$(date +%s.%N)
AOT_TIME=$(echo "$END_TIME - $START_TIME" | bc)
echo ""
echo -e "${GREEN}AOT execution time: ${AOT_TIME}s${NC}"
...
if (( $(echo "$JIT_TIME > 0" | bc -l) )); then
    PERC_DIFF=$(echo "scale=2; (($JIT_TIME - $AOT_TIME) / $JIT_TIME) * 100" | bc)
    if (( $(echo "$PERC_DIFF > 0" | bc -l) )); then
        echo -e "${GREEN}AOT is ${PERC_DIFF}% faster${NC}"
    else
        PERC_DIFF=$(echo "scale=2; -$PERC_DIFF" | bc)
        echo -e "${RED}AOT is ${PERC_DIFF}% slower${NC}"
    fi
fi