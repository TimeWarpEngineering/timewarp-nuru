...
// Runfile: sandbox/build-design-time-model.cs
// Purpose: Manually construct a RouteDefinition (design-time model) for "add {x:int} {y:int}"
// This is step-1 of task #242 - building the expected output for parser testing
// and the input for step-2 (manual runtime construction).
  ...
  handler: HandlerDefinition.ForDelegate(
    ...
    [
      ParameterBinding.FromParameter(
        ...
        isOptional: false,
        defaultValue: null,
        requiresConversion: true),  // string -> int requires conversion
      ParameterBinding.FromParameter(
        parameterName: "y",
        typeName: "global::System.Int32",
        segmentName: "y",
...
// =============================================================================
// Print the model to verify correctness
// =============================================================================

Console.WriteLine("=== RouteDefinition for 'add {x:int} {y:int}' ===");
Console.WriteLine();
Console.WriteLine($"OriginalPattern: \"{addRoute.OriginalPattern}\"");
Console.WriteLine($"MessageType: {addRoute.MessageType}");
Console.WriteLine($"Description: {addRoute.Description ?? "(null)"}");
Console.WriteLine($"ComputedSpecificity: {addRoute.ComputedSpecificity}");
Console.WriteLine($"HasRequiredParameters: {addRoute.HasRequiredParameters}");
Console.WriteLine($"HasOptions: {addRoute.HasOptions}");
Console.WriteLine($"HasCatchAll: {addRoute.HasCatchAll}");
Console.WriteLine();
...
foreach (var segment in addRoute.Segments)
{
  switch (segment)
  {
    case LiteralDefinition lit:
      Console.WriteLine($"  [{lit.Position}] Literal: \"{lit.Value}\" (specificity: {lit.SpecificityContribution})");
      break;
    case ParameterDefinition param:
      Console.WriteLine($"  [{param.Position}] Parameter: {param.PatternSyntax}");
      Console.WriteLine($"       Name: {param.Name}, Type: {param.TypeConstraint ?? "(none)"}");
      Console.WriteLine($"       CLR Type: {param.ResolvedClrTypeName}");
      Console.WriteLine($"       Optional: {param.IsOptional}, CatchAll: {param.IsCatchAll}");
      Console.WriteLine($"       Specificity: {param.SpecificityContribution}");
      break;
  }
}
Console.WriteLine();
...
Console.WriteLine($"  Kind: {addRoute.Handler.HandlerKind}");
Console.WriteLine($"  IsAsync: {addRoute.Handler.IsAsync}");
Console.WriteLine($"  ReturnType: {addRoute.Handler.ReturnType.ShortTypeName}");
Console.WriteLine($"  RequiresCancellationToken: {addRoute.Handler.RequiresCancellationToken}");
Console.WriteLine();
...
foreach (var binding in addRoute.Handler.Parameters)
{
  ...
  Console.WriteLine($"       Optional: {binding.IsOptional}, RequiresConversion: {binding.RequiresConversion}");
}
Console.WriteLine();

Console.WriteLine("=== Success! RouteDefinition constructed correctly. ===");

// =============================================================================
// Design-Time Model Types (copied from source/timewarp-nuru-analyzers/models/)
// Made public for use in this runfile.
// =============================================================================
