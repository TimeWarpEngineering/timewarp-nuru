...
public partial class NuruDelegateCommandGenerator
{
  ...
  private static string GenerateCommandClasses(List<DelegateCommandInfo> commands)
  {
    ...
    // Add provenance information for each generated class
    foreach (DelegateCommandInfo command in commands)
    {
      sb.Append(CultureInfo.InvariantCulture, $"// {command.ClassName}");
      sb.AppendLine();

      if (command.SourceFilePath is not null)
      {
        string location = command.SourceLineNumber.HasValue
          ? $"{command.SourceFilePath}:{command.SourceLineNumber}"
          : command.SourceFilePath;
        sb.Append(CultureInfo.InvariantCulture, $"//   Source: {location}");
        sb.AppendLine();
      }

      sb.Append(CultureInfo.InvariantCulture, $"//   Pattern: \"{command.PatternText}\"");
      sb.AppendLine();
      sb.Append(CultureInfo.InvariantCulture, $"//   Handler: {command.HandlerDescription}");
      sb.AppendLine();
      sb.AppendLine("//");
    }
